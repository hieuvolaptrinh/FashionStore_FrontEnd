...
export const addToCart = async (productId: number, quantity: number): Promise<CartModel> => {
  const token = localStorage.getItem('token');
  if (!token) {
    throw new Error('Không tìm thấy token xác thực');
  }
...
export const updateCartItem = async (cartDetailId: number, quantity: number): Promise<CartModel> => {
  const token = localStorage.getItem('token');
  if (!token) {
    throw new Error('Không tìm thấy token xác thực');
  }
...
export const removeFromCart = async (cartDetailId: number): Promise<CartModel> => {
  const token = localStorage.getItem('token');
  if (!token) {
    throw new Error('Không tìm thấy token xác thực');
  }
